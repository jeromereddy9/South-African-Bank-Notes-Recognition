...
# Import CLAHE for enhancement on pre-segmented images
...
# Regex pattern for filename: R10_Front_0_2012.png
...
class BanknoteDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 4. Paste note onto synthetic background 
        ...
        if self.augment and random.random() < 0.5:
            image = self._perspective_warp(image)      # Simulate camera angle
        if self.augment:
            image = self._augment_rotation_scale(image) # Rotation + scale
        if self.augment and random.random() < 0.4:
            image = self._add_gaussian_noise(image)     # Sensor noise

        # 7. Resize to fixed dimensions and normalize to [0, 1]

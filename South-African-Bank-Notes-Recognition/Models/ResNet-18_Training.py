...
# Dynamically import ResNet model
...
CONFIG = {
    ...
    "freeze_epochs": 5,         # Train  the new head first
    "total_epochs": 100,        # Total epochs 
    "lr_head": 1e-3,            # Learning rate for head warmup
    "lr_finetune": 5e-5,        # Lower LR for full fine-tuning
    "batch_size": 16,
...
def train_fold(fold_idx, train_idx, val_idx, config, device):
    ...
    # Build model (backbone frozen initially)
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimiser, T_max=config["freeze_epochs"], eta_min=1e-5
    )
...
if __name__ == "__main__":
    ...
    # Create dummy dataset to get indices
